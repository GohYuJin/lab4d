...
def main(_):
    ...
    extrinsics_from_file = np.load("database/processed/Cameras/Full-Resolution/{0}/01-canonical.npy".format(seqname1))

    # Force inst_id to 0 to avoid IndexError due to model being trained on lesser views
    # Extrinsics will be provided directly by the ground truth camera poses instead
    # Downside is that the model will use the deformations stored in camera 0
    opts["inst_id"] = 0 
    ...
    intrinsics_fr = intrinsics_fr[None].repeat(len(extrinsics_from_file), 
                                               1).view(len(extrinsics_from_file), 
                                                       4)
    Kinv = K2mat(intrinsics_fr)[frameid]
    crop2raw = torch.tensor([1.0, 1.0, 0.0, 0.0], device=model.device)
    crop2raw = crop2raw[None].repeat(frameid.numel(), 1).view(*frameid.shape, 4)
    Kmat = model.compute_render_Kmat(crop_size, crop2raw, Kinv)
    
    # batch["dataid"] = batch["dataid"] * 0
    w2c = torch.FloatTensor(extrinsics_from_file).to(model.device)[frameid]
